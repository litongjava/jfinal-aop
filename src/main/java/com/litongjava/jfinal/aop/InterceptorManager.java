...
/**
 * InterceptorManager. 1：管理控制层、业务层全局拦截器 2：缓存业务层 Class 级拦截器数组。业务层拦截器被整体缓存在
 * ProxyMethod 中 3：用于创建 Interceptor、组装 Interceptor 4：除手动 new
 * 出来的拦截器以外，其它所有拦截器均为单例
 * 
...
public class InterceptorManager {
  ...
  // 单例拦截器
  private final ConcurrentHashMap<Class<? extends AopInterceptor>, AopInterceptor> singletonMap = new ConcurrentHashMap<Class<? extends AopInterceptor>, AopInterceptor>(
      32, 0.5F);

  // 业务层 Class 级别拦截器缓存
  private final ConcurrentHashMap<Class<?>, AopInterceptor[]> serviceClassInters = new ConcurrentHashMap<Class<?>, AopInterceptor[]>(
      32, 0.5F);
  ...
  public AopInterceptor[] createServiceInterceptor(Class<?> serviceClass) {
    ...
    if (result == null) {
      AopInterceptor[] mapped = createMappedInterceptor(serviceClass);
      AopInterceptor[] before = createInterceptor(serviceClass.getAnnotation(AopBefore.class));
      ...
      if (mapped.length == 0) {
        ...
        AopInterceptor[] tmp = new AopInterceptor[mapped.length + before.length];
        System.arraycopy(mapped, 0, tmp, 0, mapped.length);
        System.arraycopy(before, 0, tmp, mapped.length, before.length);
        result = tmp;
      }
  ...
  private AopInterceptor[] doBuild(AopInterceptor[] globalInters, AopInterceptor[] routesInters,
      AopInterceptor[] classInters, Class<?> targetClass, Method method) {

    AopInterceptor[] mappedMethodInters = createMappedInterceptor(method);
    AopInterceptor[] beforeMethodInters = createInterceptor(method.getAnnotation(AopBefore.class));

    AopInterceptor[] methodInters;
    if (mappedMethodInters.length == 0) {
      methodInters = beforeMethodInters;
    } else if (beforeMethodInters.length == 0) {
      methodInters = mappedMethodInters;
    } else {
      AopInterceptor[] tmp = new AopInterceptor[mappedMethodInters.length + beforeMethodInters.length];
      System.arraycopy(mappedMethodInters, 0, tmp, 0, mappedMethodInters.length);
      System.arraycopy(beforeMethodInters, 0, tmp, mappedMethodInters.length, beforeMethodInters.length);
      methodInters = tmp;
    }
    ...
    ArrayList<AopInterceptor> result = new ArrayList<AopInterceptor>(
  ...
  public AnnotationInterceptorRegistry getAnnotationRegistry() {
    return annotationRegistry;
  }

  // 方便外部注册
  @SafeVarargs
  public final void registerInterceptor(Class<? extends Annotation> ann,
      Class<? extends AopInterceptor>... interceptors) {
    annotationRegistry.register(ann, interceptors);
  }

  public final void registerInterceptor(Class<? extends Annotation> ann,
      Class<? extends AopInterceptor> interceptor) {
    annotationRegistry.register(ann, interceptor);
  }

  private AopInterceptor[] createMappedInterceptor(java.lang.reflect.AnnotatedElement element) {
    // registry 为空时快速返回
    if (annotationRegistry.keys().isEmpty()) {
      return NULL_INTERS;
    }

    ArrayList<Class<? extends AopInterceptor>> classes = new ArrayList<>();
    for (java.lang.annotation.Annotation ann : element.getAnnotations()) {
      List<Class<? extends AopInterceptor>> mapped = annotationRegistry.get(ann.annotationType());
      if (mapped != null && !mapped.isEmpty()) {
        classes.addAll(mapped);
      }
    }
    ...
    Class<? extends AopInterceptor>[] arr = (Class<? extends AopInterceptor>[]) classes
        .toArray(new Class[classes.size()]);

    return createInterceptor(arr);
  }
